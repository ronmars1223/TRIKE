...
class GoogleMapWidgetState extends State<GoogleMapWidget>
  ...
  Future<void> _startLocationTracking() async {
    try {
      ...
      databaseRef
          .child('users/$userId/current_location')
          .onValue
          .listen((event) {
        if (!mounted) return;
          ...
          setState(() {
            ...
            if (_pickup == null || _pickup!.contains("Current Location")) {
              ...
              if (_destinationCoords != null) {
                _distance =
                    _calculateDistance(_pickupCoords!, _destinationCoords!);
              }
            }
  ...
  void _startCountdown(Duration duration) {
    ...
    _countdownTimer =
        Timer.periodic(Duration(seconds: 1), (_) => _updateCountdownText());
  }
    ...
    try {
      ...
      if (contacts.isEmpty) {
        ...
        final fallbackResponse =
            await http.get(fallbackUrl).timeout(Duration(seconds: 5));
            ...
            children: [
                ...
                onTap: () {
                  ...
                  if (_estimatedArrivalTime != null) {
                    _estimatedArrivalTime =
                        _estimatedArrivalTime!.add(Duration(minutes: 5));
                    _updateCountdownText();
                  }
                },
              ),
              ListTile(
                ...
                onTap: () {
                  ...
                  if (_estimatedArrivalTime != null) {
                    _estimatedArrivalTime =
                        _estimatedArrivalTime!.subtract(Duration(minutes: 5));
                    _updateCountdownText();
                  }
                },
              ),
  ...
  void setDestinationWithCoords(String destination, LatLng destinationCoords,
      String pickup, LatLng pickupCoords) async {
    if (!mounted) return;
  ...
  Future<void> findFastestRoute(
      [LatLng? originOverride, LatLng? destinationOverride]) async {
          ...
          if (routes.isNotEmpty) {
            ...
            for (int i = 0; i < routes.length; i++) {
              ...
              if (summary.toLowerCase().contains('highway') ||
                ...
                highwayRouteIndex = i;
                print(
                    "Highway route found at index $i: $summary"); // Debug logging
              }
            ...
            final selectedRouteIndex = (highwayRouteIndex != -1)
                ? highwayRouteIndex
                : fastestRouteIndex;
  ...
  Future<void> _fitBoundsWithMarkers() async {
    if (_routePoints.isEmpty ||
        _pickupCoords == null ||
        _destinationCoords == null) return;
    ...
    try {
      ...
      bounds = LatLngBounds(
        southwest: LatLng(
          math.min(bounds.southwest.latitude,
              math.min(_pickupCoords!.latitude, _destinationCoords!.latitude)),
          math.min(
              bounds.southwest.longitude,
              math.min(
                  _pickupCoords!.longitude, _destinationCoords!.longitude)),
        ),
        northeast: LatLng(
          math.max(bounds.northeast.latitude,
  ...
  void _calculateBasicRoute(LatLng origin, LatLng destination) {
    ...
    setState(() {
      ...
      _routeDuration = estimatedMinutes < 60
                  ...
                  child: SlideTransition(
                    ...
                    child: Container(
                      ...
                      child: Column(
                        mainAxisSize: MainAxisSize.min,
                        children: [
                          _buildMapTypeOption('Hybrid', Icons.map_outlined,
                              () => _changeMapType(MapType.hybrid)),
                          Divider(
                              height: 1,
                              thickness: 1,
                              color: Colors.grey.shade200),
                          _buildMapTypeOption('Terrain', Icons.terrain,
                              () => _changeMapType(MapType.terrain)),
                          Divider(
                              height: 1,
                              thickness: 1,
                              color: Colors.grey.shade200),
                          _buildMapTypeOption('Night Mode',
                              Icons.nightlight_round, _applyNightMode),
                        ],
                      ),
                    ),
                  ),
