import 'package:capstone_project/home/Location_input.dart';
import 'package:capstone_project/home/qr_scanner.dart';
import 'package:capstone_project/home/emergency_contact.dart'; // Add this import
...
class _HomePageState extends State<HomePage>
      ...
      if (user != null) {
        try {
          DatabaseReference emergencyRef =
              _database.ref("users/${user.uid}/emergency_alerts");
          ...
          await newAlertRef.set({
            ...
            'location': _pickupCoordinates != null
                ? {
  ...
  Future<void> _saveRideToHistory(String pickup, String destination,
      LatLng? pickupCoords, LatLng? destCoords) async {
    User? user = _auth.currentUser;
    if (user != null) {
      ...
      try {
        ...
        DatabaseReference rideHistoryRef =
  ...
  void _showQRScannerModal() {
    showModalBottomSheet(
      ...
      builder: (context) => FractionallySizedBox(
        heightFactor: 0.9,
        child: QRScannerPage(
          onDriverScanned: _handleDriverScanned,
        ),
      ),
    );
  }
